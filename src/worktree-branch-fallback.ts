...
import { expandHome, normalizePath, worktreeSlug } from './discovery.ts';
...
export async function attachExistingBranchWorktree(
  ...
  const result = await runtime.runGit(repoRoot, ['worktree', 'add', path, branch]);
  ...
  throw new Error(result.stderr.trim() || result.stdout.trim() || `git worktree add failed for branch '${branch}'`);
}
...
async function resolveRepoRoot(project: string, runtime: WorktreeBranchFallbackRuntime): Promise<string> {
  const result = await runtime.runGit(project, ['rev-parse', '--show-toplevel']);
  if (result.exitCode !== 0) {
    throw new Error(result.stderr.trim() || `failed to resolve git root for '${project}'`);
  }
