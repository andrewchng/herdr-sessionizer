...
export class WorktreeResolver {
  constructor(
    private readonly listWorktreesPorcelain: ListWorktreesPorcelain = defaultListWorktreesPorcelain
  ) {}
  ...
  async resolveExisting(
    ...
    const branchPath = existingWorktreePathFromPorcelainByBranch(
      porcelain,
      options.branch
    );
    if (branchPath) {
    ...
    const slugPath = existingWorktreePathFromPorcelainBySlug(
      porcelain,
      options.branch
    );
    if (slugPath) {
...
export function existingWorktreePathFromError(
  error: HerdrError | undefined
): string | undefined {
